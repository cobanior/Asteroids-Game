...
int main(void)
{
    ...
    *(pixel_ctrl_ptr + 1) = 0xC0000000;
    ...
    while (1)
    {
		...
		while (*score_string) {
			write_char(score_x, 2, *score_string);
			score_x++;
			score_string++;
		}
		score_x = 1;
		score_string = string_start;
		...
		else if((x_laser>=b_a1.x_pos)&&(x_laser<(b_a1.x_pos+big_x))&&(y_laser>=b_a1.y_pos)&&(y_laser<(b_a1.y_pos+big_y)))//laser hits the asteroid
		{
			clear_big_asteroid(&b_a1);
			b_a1.x_pos = 2;
            b_a1.y_pos = 2;
		...
		if(score>2){
			...
			else if((x_laser>=b_a2.x_pos)&&(x_laser<(b_a2.x_pos+big_x))&&(y_laser>=b_a2.y_pos)&&(y_laser<(b_a2.y_pos+big_y)))
			{
				clear_big_asteroid(&b_a2);
				b_a2.x_pos = 250;
				b_a2.y_pos = 2;
		...
		if(score>5){
			...
			else if((x_laser>=m_a1.x_pos)&&(x_laser<(m_a1.x_pos+med_x))&&(y_laser>=m_a1.y_pos)&&(y_laser<(m_a1.y_pos+med_y)))
			{
				clear_med_asteroid(&m_a1);
				m_a1.x_pos = 150;
				m_a1.y_pos = 2;
		...
		if(score>8){
			...
			else if((x_laser>=s_a1.x_pos)&&(x_laser<(s_a1.x_pos+small_x))&&(y_laser>=s_a1.y_pos)&&(y_laser<(s_a1.y_pos+small_y)))
			{
				clear_small_asteroid(&s_a1);
				s_a1.x_pos = 150;
				s_a1.y_pos = 230;
...
void move_ship(bool memory){
	if((current_degree == 0) && (x_ship[4] <= 315)){
		for(int i = 0; i < 57; ++i){
			x_ship[i] = x_ship[i] + 3;
		}
	}
	else if((current_degree == 90 || current_degree == -270) && (y_ship[4] >= 5)){
		for(int i = 0; i < 57; ++i){
			y_ship[i] = y_ship[i] - 3;
		}
	}
	else if((current_degree == 180 || current_degree == -180) && (x_ship[4] >= 5)){
		for(int i = 0; i < 57; ++i){
			x_ship[i] = x_ship[i] - 3;
		}
	}
	else if((current_degree == 270 || current_degree == -90) && (y_ship[4] <= 235)){
		for(int i = 0; i < 57; ++i){
			y_ship[i] = y_ship[i] + 3;
		}
	}
